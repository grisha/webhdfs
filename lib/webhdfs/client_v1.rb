  ...
  class ClientV1
    ...
    def build_path(path, op, params)
      opts = if @username and @doas
               {'op' => op, 'user.name' => @username.split('/').first, 'doas' => @doas}
             elsif @username
               {'op' => op, 'user.name' => @username.split('/').first}
             elsif @doas
               {'op' => op, 'doas' => @doas}
             else
               {'op' => op}
    ...
    def request(host, port, method, path, op=nil, params={}, payload=nil, header=nil, retries=0)
      ...
      req.proxy.user = @proxy_user if @proxy_user
      req.proxy.password = @proxy_pass if @proxy_pass
      ...
      req.headers = header.nil? ? {} : header
      req.body =  payload.nil? ? {} : payload
